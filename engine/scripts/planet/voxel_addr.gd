...
func _to_string() -> String:
	return (
		"VoxelAddr(f:%d,s:%d,cu:%d,cv:%d,lu:%d,lv:%d,lr:%d)"
...
func equals(other: VoxelAddr) -> bool:
	return (
		face == other.face
		and shell == other.shell
		and chunk_u == other.chunk_u
		and chunk_v == other.chunk_v
		and local_u == other.local_u
		and local_v == other.local_v
		and local_r == other.local_r
	)


static func from_face_col_row(
	p_face: int, p_shell: int, col: int, row: int, depth: int, chunk_size: int
) -> VoxelAddr:
